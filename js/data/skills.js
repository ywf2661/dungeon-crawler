...
export(전역): SKILLDB, CHALNA_COMBOS
...
*/
  ...
  const SKILLDB = {
    ...
    coinflip: {name:'동전 던지기', mp:3,  desc:'운명의 동전을 던진다. 50% 확률로 3배의 피해를 입히고, 50% 확률로 완전히 빗나간다',
      // 밸런스 재조정(사용자 피드백 — "너무 쎄다", 레벨1 성공시 22딜 목표).
      // mult1.4/critMult2.9(성공시 30딜)이 너무 강해서 mult1.5/critMult2.0
      // (성공시 24배 -> 방어력2 기준 22딜, 기댓값 11)으로 하향.
      type:'coinflip', mult:1.5, chance:0.5, critMult:2.0, luck:true},
    ...
    // 밸런스 조정(사용자 요청 — "전직 직전에 전직해버리면 그 보스를 못 깬다",
    // 타 2차직업 대비 딜량 시뮬레이션 최하위권) — 30%→50%로 재상향.
    mastery_purestrike: {name:'순일격', mp:0, type:'passive',
      desc:'기본 공격의 피해가 항상 50% 증가한다.'},
    // 재설계(사용자 요청 — "때릴 때마다 딜이 조금씩 더 세지는 느낌, 방어나 다른
    // 행동을 하면 다시 돌아오고"): "2타마다 발동"에서 연속 타격 스택형으로 변경.
    // 실제 수치는 player-actions.js의 playerAttack() 참고(스택당 +10%, 최대
    // 5스택/+50%, 방어·아이템 사용 시 초기화).
    warriorPuristEcho: {name:'메아리 타격', mp:0, type:'passive',
      desc:'기본 공격을 연속으로 낼 때마다 점점 강해진다(최대 5연속, +50%). 방어하거나 다른 행동을 하면 흐름이 끊겨 처음부터 다시 쌓인다.'},
    ...
    // 전사 - 찰나검사(warrior_chalna) — 전사의 3번째 분기(사용자 기획).
    // 완박/중박/급박 3종을 "즉시 시전"(type:'chalnaStrike') 또는 "예약"
    // (type:'chalnaReserve')으로 쓸 수 있다. 예약하면 이번 턴은 피해 없이
    // battleFlags.chalnaReserve = {beat, turnsLeft:1}만 세팅하고 턴을 마친다.
    // 이 상태에서 다음 내 턴에 chalnaStrike 타입 스킬을 쓰면(즉시시전이든 새로
    // 예약을 걸려던 것이든) 두 beat가 합쳐진 콤보 효과가 대신 발동하고
    // (완전대체 — 원래 그 스킬의 개별 효과는 나가지 않음) chalnaReserve는 소모된다.
    // 콤보 조합 데이터는 아래 CHALNA_COMBOS(beat 두 개를 정렬한 문자열 키,
    ...
    chalnaTriBeat: {name:'삼박난무', hanja:'三拍亂舞', mp:16, cooldown:3, desc:'세 박자를 한꺼번에 몰아쳐 어지럽게 베어낸다.',
    ...
    mastery_timewarp: {name:'시간 왜곡', mp:0, type:'passive',
      desc:'매 턴 20% 확률로 시간이 뒤틀려, 적의 턴을 건너뛰고 자신이 한 번 더 행동한다(자동 발동). 가끔 그 확률이 유난히 짙게 몰릴 때가 있다 — 마치 그 순간만큼은, 진짜 시간의 주인이 손을 들어준 것처럼.'},
    ...
    mageTimeParadox: {name:'시간의 역설', mp:18, cooldown:3, type:'timeparadox', baseMult:1.0, stackMult:0.6,
      desc:'쌓아온 시간 조각을 모두 소비해 그 힘을 한꺼번에 무너뜨려 적을 덮친다. 조각이 많을수록(최대 5개) 압도적으로 강력하다. 무너지는 시간의 파편 사이로, 아주 희미하게 낯선 이름 하나가 스친다 — \'Aiōn\'이라고 읽히는 것도 같다.'},
    ...
    // ---------- [리뉴얼] 역병숙주(rogue_alchemist, 구 "독사") 스킬 재설계 ----------
    // 구버전("독 중첩"+"맹독 주입")은 마스터리가 기본공격만으로도 자동으로
    // +1스택을 쌓아버려서, 액티브(맹독 주입)는 그 위에 +1 더 얹는 것뿐이라
    // 레벨15(삼중 주입, +3)를 배우기 전까지는 쓸 이유가 없는 죽은 스킬이었다.
    // 사용자 피드백으로 "기생형" 컨셉으로 전면 교체 — 마스터리(적 약화)는
    // 그대로 자동으로 굴러가고, 액티브는 "그 약화분을 내가 훔쳐온다"는
    // 별개의 축을 담당한다(스택을 소모/폭발시키는 방식이 아님 — 저주술사/
    // 폭주화부의 "모아서 터뜨리기"와 겹치지 않도록 의도적으로 다른 결로 설계).
    //
    // 마스터리 "역병 잠식": 세 가지 역할.
    // (1) enemy.venomStacks(0~10, 전투가 끝날 때까지 유지 — 일반 dot과 달리
    //     턴이 지나도 사라지지 않음)를 매 라운드 자동으로 틱시키는 패시브
    //     피해원(combat/enemy-turn.js의 enemyTurnReal(), 기존 dot 연출
    //     파이프라인 재사용).
    // (2) 기본 공격과 그 외 모든 도적 스킬이 적중할 때마다 스택을 +1 쌓는다
    //     (체액 흡수 자신은 이 범용 훅을 타지 않고 별도의 더 큰 자기 몫을 준다
    //     — 아래 rogueVenomInject 참고). combat/player-actions.js의
    //     playerAttack()/범용 phys·magic 분기/multihit 분기 세 곳에 이 훅이
    //     들어가 있다(기존 그대로 유지).
    ...
    // 스택 수는 화면에서 적 상태 배지 영역에 "☠ 역병중첩 N/10"으로 표시된다
    ...
    // 레벨10 액티브 "체액 흡수": 피해를 입히는 동시에 자기 전용 잠식 스택을
    // "추가로 더" 쌓고(마스터리의 범용 +1과는 별개 — 레벨10부터 기본 3,
    // 사용자 요청으로 상향 — 구버전은 기본 1/레벨15 3이었음), 그렇게 갱신된
    // 직후의 스택 수의 절반(내림, 레벨15를 배웠다면 전부)만큼 내 공격력을
    // 이번 전투 동안 영구히 흡수한다. 스택을 소모하지 않으므로 몇 번을 써도
    // 마스터리의 dot/디버프는 그대로 유지된 채 계속 흡수만 누적된다.
    // combat/player-actions.js의 'venominject' 타입 핸들러, combat/
    // enemy-turn.js의 getVenomAbsorbBonus()(battleFlags.venomAbsorbPoints를
    // effectiveAtk()/effectiveMag()에 반영, 최대 +30%p 안전장치) 참고.
    rogueVenomInject: {name:'체액 흡수', mp:6, type:'venominject', mult:1.25,
      desc:'적의 상처에서 역병의 체액을 빨아들인다. 피해와 함께 잠식 스택이 추가로 3 더 쌓이며, 그 직후 스택 수의 절반(레벨15를 배웠다면 전부)만큼 내 공격력을 이번 전투 동안 영구히 흡수한다.'},
    // 레벨12 패시브 "만성 기생": 마스터리 dot이 적을 갉아먹을 때마다 그
    // 피해의 일부를 그대로 내 체력으로 되돌린다(라이프스틸). 별도 조작 없이
    // 매 라운드 자동 발동 — combat/enemy-turn.js의 enemyTurnReal() dot 계산
    // 직후에서 처리.
    rogueVenomRefine: {name:'만성 기생', mp:0, type:'passive',
      desc:'역병이 적을 갉아먹을 때마다 그 생명력의 일부가 나에게 흘러들어온다. 매 라운드 잠식 피해량의 20%만큼 자동으로 체력을 회복한다.'},
    // 레벨15 패시브 "완전 기생화": [수정] 레벨10 체액 흡수의 자기 전용 스택
    // 보너스가 3으로 상향되면서(사용자 요청) 더 이상 이 스킬이 스택 증가량에
    // 관여하지 않는다 — 이제 순수하게 흡수 비율만 절반(내림)에서 전부로
    // 올려주는 역할. player-actions.js의 rogueVenomInject 핸들러에서 이
    // 패시브 보유 여부를 확인한다.
    rogueVenomTriple: {name:'완전 기생화', mp:0, type:'passive',
      desc:'기생이 완전해진다. 체액 흡수로 쌓은 잠식 스택을 절반이 아니라 전부 공격력으로 흡수한다.'},
    ...
    // 않는다(스노우볼 방지).
    // 밸런스 조정(사용자 요청 — 도적 타 2차직업 대비 딜량 시뮬레이션 결과, 다른
    // 도적 분기 대비 절반 수준으로 낮게 나옴). 원인 분석: 강령 소환 자체가
    // 데미지 0인데도 턴을 소모해, 소환한 턴은 완전히 손해였다. 분신 배가
    // (rogueDoubleImage)와 동일하게 턴을 소모하지 않도록 바꿔, 소환 직후 같은
    // 턴에 바로 원혼의 명령까지 이어붙일 수 있게 했다(player-actions.js의
    // necrosummon2 분기 — enemyTurn() 호출 제거).
    // 밸런스 조정(사용자 요청 — "스킬 마나소모량을 전체적으로 줄여줘") — 12→8.
    necroSummon: {name:'강령 소환', mp:8, type:'necrosummon2', summonTurns:4,
      desc:'소환 계약으로 지정한 몬스터를 4턴간 불러내 자동으로 공격하게 한다(이미 소환되어 있으면 지속시간만 갱신). 턴을 소모하지 않아, 곧바로 다른 스킬로 이어갈 수 있다.'},
    // 레벨12 액티브(사용자 요청 — "소환한 몬스터의 스킬을 주인이 쓸 수 있었으면
    // 좋겠다"는 피드백으로 기존 패시브 강령 증폭을 대체). 소환수에게 즉시 한 번
    // 더 공격을 명령한다. turnsLeft는 깎지 않아(자동 틱과 별개) MP만으로
    // 스팸을 제한한다. 실제 처리는 player-actions.js의 necroCommand 분기.
    // 밸런스 조정(사용자 요청) — MP 15→10→8로 추가 인하해 8턴 기준 캐스팅
    // 횟수를 더 늘렸다.
    necroEmpower: {name:'원혼의 명령', mp:8, type:'necroCommand',
      desc:'소환수에게 즉시 한 번 더 공격을 명령한다. 자동 공격과 달리 소환수의 특성(흡혈/강타/저주전이/회복)을 전부 확정 발동시킨다.'},
    // 레벨15 액티브(위와 같은 요청으로 자동 발동 패시브에서 교체) — 소환수를
    // 즉시 소멸시키며 남은 지속시간을 전부 한 방의 폭발력으로 바꾼다. 언제
    // 터뜨릴지 직접 고르는 게 핵심이라 자연 소멸을 기다리지 않는다.
    // 밸런스 조정(사용자 요청) — MP 18→14로 인하.
    necroLastRites: {name:'혼백 해방', mp:14, type:'necroRelease', releaseMult:1.2,
      desc:'소환수를 즉시 해방시켜, 남은 지속시간 전부를 한 방의 폭발 피해로 바꾼다(특성도 이 폭발에 한 번씩 강하게 발동).'},
  };

  // 찰나검사(warrior_chalna) 콤보 6종 — beat 두 개를 알파벳순으로 정렬해
  // 합친 키('fast'<'mid'<'slow' 순서이므로 예: 완박+급박 → 'fast+slow').
