...
*/
  ...
  const SKILLDB = {
    ...
    // 레벨12 "분신 배가": 토글형이지만 혈서/원소계약과 달리 마나를 환불하지
    // 않는다(사용자 요청 — 켜고 끌 때마다 실제로 마나가 든다). 턴은 소모하지
    // 않으므로, 같은 턴에 이어서 다른 공격 스킬을 쓸 수 있다(그 스킬의 마나도
    // 별도로 든다). 켜져 있는 동안 공격형 스킬이 적중할 때마다 분신이 즉시 한
    // 번 더 나타나 추가 타격을 가하며(잔영 마스터리의 "적 턴 직전 예약타"와는
    // 별개로, 이건 그 자리에서 바로 터짐), 위력은 battleFlags.doubleImageStacks
    // (0~2, 최대 2)에 따라 40%→60%→80%로 강해진다. 스택은 반드시 "이번 공격의
    // 즉시타 계산 이후"에 오른다(사용자 확정) — 그래서 토글만 켜고 바로 다른
    // 스킬(예: 백귀야행)을 쓰면 그 시점 스택은 여전히 0이다. 새 타입
    // 'doubleimagepact'로 처리한다.
    rogueDoubleImage: {name:'분신 배가', mp:6, type:'doubleimagepact',
      desc:'환영을 증폭하는 술식을 건다(토글, 턴을 소모하지 않음). 켜져 있는 동안 공격이 적중할 때마다 분신이 즉시 한 번 더 나타나 추가 타격을 가한다. 위력은 켜둔 채로 공격할수록 강해진다(최대 2단계). 다른 토글과 달리 켜고 끌 때마다 마나가 실제로 든다.'},
    // 레벨15 궁극기 "백귀야행": 이번 전투에서 잔영(마스터리)이 발동했던 누적
    // 횟수(battleFlags.afterimageTriggerCount — 이번에 새로 추적)만큼 분신이
    // 동시에 몰아친다. 확정 발동 마스터리 기준으로 세는 것이라 "운"이 아니라
    // "이번 전투에서 공격형 스킬을 얼마나 적극적으로 썼는가"에 정직하게 비례한다.
    // 사용 후 카운트는 0으로 초기화된다. 새 타입 'nightparade'로 처리한다.
    rogueUndeadParade: {name:'백귀야행', mp:20, type:'nightparade',
      desc:'이번 전투에서 쌓아온 잔영의 흔적을 모두 불러내, 그 횟수만큼 분신 군단이 동시에 적을 몰아친다. 오래, 적극적으로 싸울수록 압도적으로 강력해진다.'},
