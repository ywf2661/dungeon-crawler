...
*/
  ...
  const SKILLDB = {
    ...
    // "추가로 더" 쌓고(마스터리의 범용 +1과는 별개 — 기본 1, 레벨15를 배우면
    // 3, 구 "맹독 주입"과 수치 동일하게 유지), 그렇게 갱신된 직후의 스택
    // 수의 절반(내림, 레벨15를 배웠다면 전부)만큼 내 공격력을 이번 전투
    // 동안 영구히 흡수한다. 스택을 소모하지 않으므로 몇 번을 써도 마스터리의
    // dot/디버프는 그대로 유지된 채 계속 흡수만 누적된다.
    ...
    rogueVenomInject: {name:'체액 흡수', mp:6, type:'venominject', mult:1.25,
      desc:'적의 상처에서 역병의 체액을 빨아들인다. 피해와 함께 잠식 스택이 추가로 1(레벨15를 배웠다면 3) 더 쌓이며, 그 직후 스택 수의 절반(레벨15를 배웠다면 전부)만큼 내 공격력을 이번 전투 동안 영구히 흡수한다.'},
    ...
    // 레벨15 패시브 "완전 기생화": 체액 흡수의 자기 전용 스택 보너스가
    // 1 -> 3으로 늘고(레벨12 만성 기생과 별개로 항상 함께 적용 — 즉 체액
    // 흡수 한 번으로 마스터리 +1과 합쳐 최종 +4까지 쌓일 수 있음, 구 "삼중
    // 주입"과 스택 증가 수치는 동일하게 유지), 흡수 비율도 절반이 아니라
    // 전부로 상승한다. player-actions.js의 rogueVenomInject 핸들러에서 이
    // 패시브 보유 여부를 확인한다.
    rogueVenomTriple: {name:'완전 기생화', mp:0, type:'passive',
      desc:'기생이 완전해진다. 체액 흡수를 쓸 때마다 잠식 스택이 3개씩(기본 1개 대신) 더 쌓이고(최대 10은 동일), 그 스택을 절반이 아니라 전부 공격력으로 흡수한다.'},
