...
*/
  ...
  */
  const EPIC_JOB_ENCHANTS = {
    ...
    re_venomrush: {slot:'weapon', specId:'rogue_alchemist', name:'폭주 주입 각인', prefix:'폭주하는', exclusiveGroup:'alchemist_weapon',
      desc:'체액 흡수의 자기 전용 스택 보너스가 2배가 된다. 대신 독 스택 상한을 넘기면 초과분만큼 자신도 반동 피해를 입는다. (무기 각인은 이것과 폭식의 각인 중 하나만 고를 수 있다)'},
    re_venomburst: {slot:'armor', specId:'rogue_alchemist', name:'폭발 정제 각인', prefix:'폭발하는', exclusiveGroup:'alchemist_armor',
      desc:'독 스택이 최대치에 도달한 라운드엔 평소 틱딜 대신 스택을 전부 소모하는 큰 폭발 피해로 터진다. 대신 스택당 지속피해가 낮아진다. (방어구 각인은 이것과 잠식 갑주 중 하나만 고를 수 있다)'},
    re_solovenom: {slot:'accessory', specId:'rogue_alchemist', name:'고독 각인', prefix:'고독한', exclusiveGroup:'alchemist_accessory',
      desc:'체액 흡수의 자기 전용 스택 보너스가 한 번 더 2배가 된다(삼중 주입과 겹치면 3개→12개). 독 스택 상한이 10→6으로 줄어드는 대신, 스택 하나하나의 매 라운드 피해량이 +25% 늘어난다. (장신구 각인은 이것과 공생의 각인 중 하나만 고를 수 있다)'},
    // ---------- 역병숙주 신규 각인 3종(사용자 기획, 이번 세션) ----------
    // 위 3종(폭주 주입/폭발 정제/고독)과 같은 슬롯에서 exclusiveGroup으로
    // 하나만 고르는 대안 각인. "스노우볼 가속(스택딜)" 축인 기존 3종과 달리,
    // 이쪽은 각각 "초반 스노우볼/적 약화/생존" 축으로 설계했다(사용자 승인).
    re_gluttony: {slot:'weapon', specId:'rogue_alchemist', name:'폭식의 각인', prefix:'폭식하는', exclusiveGroup:'alchemist_weapon',
      desc:'체액 흡수가 쌓는 자기 전용 잠식 스택이 3에서 5로 늘어난다. 대신 체액 흡수의 즉발 피해가 25% 줄어든다. (무기 각인은 이것과 폭주 주입 각인 중 하나만 고를 수 있다)'},
    re_corrosion: {slot:'armor', specId:'rogue_alchemist', name:'잠식 갑주', prefix:'잠식된', exclusiveGroup:'alchemist_armor',
      desc:'역병 잠식의 적 공격력·방어력 약화가 스택당 2%→3%(최대 -30%)로 커진다. 대신 역병 잠식의 매 라운드 지속피해가 25% 줄어든다. (방어구 각인은 이것과 폭발 정제 각인 중 하나만 고를 수 있다)'},
    re_symbiosis: {slot:'accessory', specId:'rogue_alchemist', name:'공생의 각인', prefix:'공생하는', exclusiveGroup:'alchemist_accessory',
      desc:'만성 기생의 라이프스틸 비율이 20%→35%로 오른다. 대신 독 스택 상한이 10→7로 줄어든다. (장신구 각인은 이것과 고독 각인 중 하나만 고를 수 있다)'},
