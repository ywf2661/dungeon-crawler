...
- 이전 세션들(아이온 신설, 스토리 통합, 독사 밸런스) 상세는 `HANDOFF.md`로
  이동 — **독사의 구조적 한계(checkBattleEnd 타이밍 이슈)는 여전히 미해결.**
...
### 4순위 — 남은 SFX 슬롯(guard/poisonHit/fail/click/victory/gameOver/
  statusApply) 및 마을/일반전투 BGM 추가 음원 확보 여부 확인
- 사용자가 "따로 구해볼게"라고 한 항목들 — 준비되면 파일만 넣으면 됨

### 5순위 — UI 리디자인 확장 여부 논의
- 이번엔 폰트/버튼만 손댔음. 카드류(location-card 등)의 그라데이션
  배경까지 평면화할지는 별도 논의 필요(요청받지 않아 보류함)
```
